...
const availableModels: ModelOption[] = [
  ...
  },
  {
    ...
    name: "Qwen 3.6 35B A3B Uncensored (NanoGPT)",
    shortName: "Qwen 35B",
    provider: "nano-gpt",
  },
  {
  ...
  },
  {
    ...
    name: "Hermes 3 Llama 3.1 70B (OpenRouter)",
    shortName: "Hermes 3",
    provider: "openrouter",
  },
  {
...
// (Pure cloud: no localStorage keys — Supabase is the only persistence layer.)
...
export default function Page() {
  ...
  // User-added models from the Add Model form. Synced to Supabase (user_prefs).
  ...
  // No localStorage: Supabase is the only source of truth. Mark ready on mount
  // so the cloud sync can run; chats/prefs are loaded purely from the cloud.
  useEffect(() => {
    setLoaded(true);
  }, []);

  // Load chats from Supabase (the only source of truth) once the user is signed
  // in. A successful read is authoritative; a failed read shows nothing and
  // retries on the next load (no local cache).
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // (empty cloud → the auto-create effect makes a starter chat)
        ...
        if (active) {
          // Cloud read failed: no local cache anymore, so show empty + a clear
          // message and do NOT auto-create/write (cloudReadOk stays false).
          setSyncStatus(
            `⚠ couldn't reach the server — retrying on reload (${e instanceof Error ? e.message : String(e)})`,
          );
        }
      } finally {
  ...
  function addCustomModel(): { ok: true } | { ok: false; error: string } {
    ...
    const model: ModelOption = {
      id: trimmed,
      name: `${trimmed} (${newModelProvider === "nano-gpt" ? "NanoGPT" : "OpenRouter"})`,
      shortName,
      ...
      {syncStatus && (
        <div
          style={{
            ...
            color: syncStatus.startsWith("⚠") ? "#ffb3a8" : "#9ed68c",
              ...
              <div className="add-model-form">
                <div className="input-row">
                  ...
                  <Dropdown
                    ...
                    options={[
                      { value: "nano-gpt", label: "NanoGPT" },
                      { value: "openrouter", label: "OpenRouter" },
                    ]}
                    onChange={(v) =>
                      setNewModelProvider(v as "nano-gpt" | "openrouter")
                    }
                    menuAlign="left"
